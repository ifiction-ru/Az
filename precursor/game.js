...
//----------
/*НеДаёмВзятьСундук2 = new РеакцияНаСобытие(События.Переместить, {
    'место': Остров,
    'что':   Сундук,
    'куда':  ИГРОК
    }, function(Данные) {
        Вывести('<p>Сундук глубоко вкопан. Просто так не вытащить из '+Данные.откуда.ID+'.</p>');
        return НЕТ;
    }
);*/
НеДаёмНичегоСделать = new РеакцияНаСобытие(События.Действие, {'что':[Сундук, Остров]}, function(Данные) {
    Вывести('<p>Пока ничего нельзя сделать.</p>');
    return НЕТ;
});
НеДаёмНичегоСделать.Включить();
//----------
НеДаёмВзятьСундук = new РеакцияНаСобытие('ПробуемПоднятьСундук', function(Данные) {
    Вывести('<p>Сундук глубоко вкопан. Просто так не вытащить из '+Данные.КтоТащит.ID+'.</p>');
    return НЕТ;
});
НеДаёмВзятьСундук.Включить();
//EVENTS.updateAfterLoad();
//var a = НеДаёмВзятьСундук.isActive();
//----------
/*РеакцияНаСобытие({'событие':События.Переместить, 'место':Остров, 'что':Сундук, 'куда':ИГРОК}, function(Данные) {
...
// СУНДУК
    ...
    //----------
    Сундук.Свойство('Открыт', 1);
    Пистолет.Свойство('Открыт', 2);
    //----------
    //ОткрытЛиСундук = new РеакцияНаСобытие(События.Свойство, {'свойство':'Открыт', 'что':Пистолет}, function(Данные) {
    //    return Данные.значение+100;
    //});
    //ОткрытЛиСундук.Включить();
    //alert('Сундук:'+Сундук.Открыт+', Пистолет: '+Пистолет.Открыт);
    //----------
