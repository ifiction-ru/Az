...
/* --------------------------------------------------------------------------- */
// ВСЯКОЕ
    var ОткудаИнвентарь = 'У Кутуша своеобразное чувство юмора: он оставил вам верёвку, чтобы повеситься, саблю, чтобы заколоться, и пистолет, чтобы застрелиться, когда будет совсем невмоготу.';
    //----------
...
// ПЕРСОНАЖИ
    ...
    // ИГРОК
        ...
        //----------
        ИГРОК.Описание('игрок', 'Среди берегового братства вы известны своей удачливостью. И эта ситуация испытание для репутации, да.');
        //----------
        ...
        ИГРОК.Действие([
            ...
            //----------
                ...
                Вывести('Тут нет зеркала. А тащиться к воде, только для того, чтобы собой любоваться, - это как-то не по-пиратски.');
            }
        ); // действие
        //----------
    ...
    // ТУШИТО КУТУШ
        ...
        Кутуш.Действие('приманить кутуша', [
            {где:ВЕЗДЕ, глагол:['махать','сигналить','привлечь'], А:Кутуш},
            //----------
            ], function() {
        ...
        Кутуш.Действие('пальба', [
            {где:ВЕЗДЕ, глагол:['выстрелить'], А:[Кутуш], Б:[null,Пистолеты]},
            //----------
            ], function() {
...
// ЛОКАЦИИ
    ...
    // КОРАБЛЬ
        ...
        //----------
        Корабль.Действие([
            {где:ВЕЗДЕ, глагол:['махать','сигналить','привлечь'], А:Корабль},
            //----------
            ], function() {
                if (Карта.Находится(ИГРОК) == НЕТ) {
                    Вывести('Да хоть колесом на руках ходи! Кутуш не такой дурак, чтобы вернуться только для того, чтобы спросить, не напекло ли мне голову.');
...
// ОБСТАНОВКА
    ...
    // ПАЛЬМА
        ...
        //----------
        Пальма.Действие([
            {где:ВЕЗДЕ, глагол: ['сесть', 'лечь', 'устроиться', 'прятаться'], А: [null, Пальма, 'тени', {предлоги:'в', слова:'тень'}, {предлоги:'от', слова:'солнца'}]},
            {где:ВЕЗДЕ, А: ['тень', {предлоги:'в', слова:['тень','тени']}, {предлоги:'от', слова:'солнца'}]},
            //----------
            ], function() {
                if (ИГРОК.СидитПодПальмой == ДА) {
                    ...
                    if (ИГРОК.Находится(ПодПальмой) == НЕТ) {
                        ...
                        Вывести('По щиколотку утопая в мягком песке, вы подошли к пальме и присели в её тени.');
                    } else {
                        Вывести('Вы присели в тени под пальмой, облокотившись спиной о ствол.');
                    } // end if
                    //----------
        ...
        //----------
        Пальма.Действие([
            {где:ВЕЗДЕ, глагол:['осмотреть', 'изучить'], А:['царапины', 'царапину', 'борозды', 'борозду']},
            //----------
            ], function() {
...
// ПРЕДМЕТЫ
    ...
    // ПИСТОЛЕТ
        ...
        //----------
        Пистолет.Действие([
            {глагол: 'выстрелить', А:[Корабль,Пальма,Сундук,Замок], Б:[null,Пистолет]},
            //----------
            ], function() {
    ...
    // ВЕРЁВКА
        ...
        Верёвка.Действие([
            ...
            //----------
                ...
                if (Сундук.Находится(ПодПальмой) == ДА) {
                    ...
                    Вывести('Посмотрев по сторонам, вы ничего не нашли такого, чтобы привязать.');
                }// end if
            }
        ); // действие
        //----------
        Верёвка.Действие([
            ...
            //----------
            function() {
                if (Фитиль.Находится(ИГРОК) == ДА || Сундук.Открыт == ДА) {
    ...
    // МЕШОЧЕК
        ...
        //----------
        Мешок.Действие([
            {глагол:['взять','достать','вытащить'], А:ЛюбоеСлово, Б:[Мешок]},
            //----------
            ], function(Команда) {
                if (Письмо.Прочитали == НЕТ) {
                    ...
                    if (Команда.А.Объект == null || Команда.А.Объект.Это(Карта) == НЕТ) {
                        ...
                        if (Карта.Находится(ИГРОК) == ДА) {
                            ...
                            //----------
                            Корабль.ВключитьДействие('показать карту');
                            Кутуш.ВключитьДействие('показать карту');
                            Карта.ВключитьДействие('показать карту');
                            //----------
    ...
    // КАРТА
        ...
        function СигналКораблю () {
            ...
            Корабль.ВыключитьДействие('показать карту');
            Кутуш.ВыключитьДействие('показать карту');
            Карта.ВыключитьДействие('показать карту');
            //----------
        ...
        Кутуш.Действие('показать карту', [
            {где:ВЕЗДЕ, глагол:['махать'], А:[Кутуш], Б:[Карта]},
            {где:ВЕЗДЕ, глагол:['сигналить'], А:[Кутуш], Б:[Карта]},
            {где:ВЕЗДЕ, глагол:['показать'], А:[Карта], Б:[Кутуш]},
            {где:ВЕЗДЕ, глагол:['привлечь'], А:[Кутуш], Б:[Карта]},
            //----------
            ], function() {
                СигналКораблю();
            }
