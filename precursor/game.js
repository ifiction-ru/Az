...
// ПЕРСОНАЖИ
    ...
    // ИГРОК
        //----------
        ИГРОК.Описывается(['я', 'игрок'], null, ['е']);
        //----------
        ИГРОК.Описание('игрок', '');
        //----------
        ...
        //----------
        var ОписаниеГероя = '\
            Вас зовут Пекка Уоткин, и в детстве вы мечтали стать викингом. Перебравшись в юности в тёплые края, вы обнаружили, что быть пиратом куда веселее: климат лучше, и не нужно значительную часть года сидеть в холодном доме.\n\n\
            \n\n\
            С тех пор прошло несколько лет, и вы успели поплавать во многих морях. Своим кораблём пока не обзавелись, но надежда умирает, как говорится, последней. И сегодня не этот день, нет.';
        //----------
        ИГРОК.Действие({глагол:'кто', А:ИГРОК}, function() {
            Вывести(ОписаниеГероя);
        });
        //----------
        ...
        //----------
        ИГРОК.Действие([
            {глагол: 'осмотреть', А:['камзол','одежда','брюки','штаны']},
            //----------
            ], function() {
                Вывести('Для этих краёв вы одеты даже несколько элегантно. Впрочем, в некоторых местах наличие камзола и уж тем более брюк - обязательное требование к уважаемому человеку.');
            }
        ); // действие
        //----------
        ИГРОК.Действие([
            ...
            {А:[{предлоги:['на', 'к'], слова:['север','юг','запад','восток']}]},
            //----------
            ], function() {
    ...
    // ТУШИТО КУТУШ
        ...
        Кутуш.Описывается(['кутуш', 'тушито'], null, ['е']);
        //----------
        Кутуш.Описание('Тушито Кутуш', 'Местный пират с сомнительной репутацией. Вы рискнули связаться с ним только из-за его корабля. За что и поплатились.');
        //----------
        ...
        Кутуш.Действие([
            ...
            //----------
                ...
                if (Кутуш.РасстояниеДоОстрова == 5) {
                    Вывести('Отсюда его не видно. Но сам он наверняка периодически поглядывает в сторону острова. Чем ему ещё заняться в штиль?');
                } else {
        ...
        Кутуш.Действие('пальба', [
            {где:ВЕЗДЕ, глагол:'стрелять', А:[Кутуш,'пират'], Б:[null,Пистолет,Пистолеты]},
            //----------
            ], function() {
                СтрельбаИзПистолетов(Кутуш);
            }
        ); // действие
        //----------
    //--------------------------------------------------
    // ГРОМИЛЫ
        ...
        Громилы.Действие('пальба', [
            {где:ВЕЗДЕ, глагол:'стрелять', А:[Громилы], Б:[null,Пистолет,Пистолеты]},
            //----------
            ], function() {
                СтрельбаИзПистолетов(Громилы);
            }
        ); // действие
    //--------------------------------------------------
/* --------------------------------------------------------------------------- */
// ЛОКАЦИИ
    //----------
    // КОРАБЛЬ
        Корабль.Описывается(['корабль'], null, ['е']);
        Корабль.СкрыватьСодержимое = ДА;
        Корабль.СодержимоеИзвестно = ДА;
        //----------
        Корабль.Свойство('ЗаметилиСуматоху', НЕТ);
        //----------
        Корабль.Описание('Корабль', function () {
            ...
            } else if (Карта.Находится(ИГРОК) == ДА && Кутуш.РасстояниеДоОстрова == 5) {
                result = result+'\n\n\n\nКажется, на корабле поднялась суматоха. Периодически у борта замирают силуэты людей - неужели вас высматривают?..';
                Корабль.ЗаметилиСуматоху = ДА;
            } // end if
            return result;
        });
        //----------
        ...
        ИГРОК.Действие([
            ...
            //----------
                ...
                Вывести('Для такого опытного пловца как вы, расстояние до корабля не проблема. Однако тут полно всяких морских гадов, и вас съедят, не успеете проплыть и полпути.');
            }
        ); // действие
        //----------
        Корабль.Действие([
    ...
    //--------------------------------------------------
    // БЕРЕГ
        Берег.Описывается(['берег', 'побережье'], null, ['е']); //
        //----------
        ...
        Берег.Действие([
            {глагол: 'осмотреть', А:['воду','океан']}, // осмотреть берег
                ...
                Вывести('Вода, кругом вода. В здешних краях в воде водится всякая круглосуточно голодная живность. Лучше держаться от неё подальше.');
            }
        ); // действие
        //----------
        Берег.Действие([
...
// ОБСТАНОВКА
    ...
    // ПАЛЬМА
        ...
        Пальма.Действие([
            ...
            {где:ВЕЗДЕ, глагол:'осмотреть', А:Ствол, В:Пальма},
            //----------
            ], function(Команда) {
        ...
        // Встать
        Пальма.Действие([
            {где: ПодПальмой, глагол:['встать','подняться']}, // +++ А:[null,'ноги']
            //----------
            ], function() {
        ...
        //----------
        function ИзучитьСтволПальмы () {
            ИГРОК.СидитПодПальмой = НЕТ;
            if (Пальма.ИзучилиРисунок == ДА) {
                Вывести('Больше ничего интересного вы не обнаружили.');
            } else {
        ...
        // Изучить ствол, кору
        Пальма.Действие([
            {где:ВЕЗДЕ, глагол:'изучить', А:[Пальма, Ствол, 'кора'], В:Пальма},
            {где:ВЕЗДЕ, глагол:'изучить', А:'кора'},
            //----------
            ], function() {
        ...
        Пальма.Действие([
            ...
            {где:Берег, А:{предлоги:['к','под'], слова:Пальма}},
            //----------
            ], function() {
        ...
        Пальма.Действие([
            ...
            {где:ВЕЗДЕ, глагол:['лезть','карабкаться'], А:Ствол, Б:[null,Пальма]},
            //----------
            ], function() {
    ...
    //--------------------------------------------------
    // СТВОЛ
        Ствол.Описывается(['ствол'], null, ['е']);
        //----------
        Ствол.Свойство('Контекст', Пальма);
        //----------
        Ствол.СкрыватьВСодержимом = ДА;
        //----------
        Ствол.Поместить(ПодПальмой);
        //----------
        Ствол.Действие([
            {где:ВЕЗДЕ, А:Ствол},
            {где:ВЕЗДЕ, глагол:['осмотреть','изучить'], 'А':Ствол},
            //----------
            ], function() {
                if (Ствол.Контекст.Это(Пальма)) {
                    if (Пальма.НащупалиРисунок == НЕТ) {
                        Вывести('Ствол пальмы возвышается над островом метров на пятнадцать.');
                    } else {
                        ИзучитьСтволПальмы();
                    } // end if
                } else if (Ствол.Контекст.Это(Пистолет)) { // end if
                    Вывести('Вы покрутили пистолет в руках, пытаясь заглянуть в ствол. Ничего не видно.');
                } else {
                    Вывести('Серьёзное оружие, вне всякого сомнения.');
                } // end if
            });
        //----------
        var КонтекстСтвола = new РеакцияНаСобытие(События.Действие, {'когда':События.После, 'что':[Пистолет,Пистолеты,Пальма]}, function(Данные) {
            Ствол.Контекст = Данные.что;
        });
        КонтекстСтвола.Включить();
        //----------
...
// ПРЕДМЕТЫ
    ...
    // ПИСТОЛЕТ
        ...
        //----------
        Пистолет.Описывается(['пистолет'], null, ['е']);
        //----------
        ...
        Пистолет.Действие([
            {А: Пистолет},
            {глагол:['осмотреть','изучить'], А:Пистолет},
            //----------
            ], function() {
        ...
        //----------
        Пистолет.Действие([
            {глагол:['осмотреть','изучить'], А:[Ствол,'дуло'], В:Пистолет},
            //----------
            ], function() {
                Вывести('Вы покрутили пистолет в руках, пытаясь заглянуть в ствол. Ничего не видно.');
            }
        ...
        //----------
        Пистолет.Действие([
            {глагол:'стрелять', А:[Корабль,Пальма,Сундук,Замок,Шлюпка,Кутуш,'пират'], Б:[null,Пистолет]},
            //----------
            ], function() {
    ...
    // ПОРОХ
        ...
        Порох.Действие([
            ...
            //----------
                ...
                if (Пистолет.Находится(ИГРОК) == НЕТ) {
                    ...
                    if (Порох.Находится(Пистолет) == ДА) {
                        if (Команда.Б.Объект == null) {
                            Вывести('Мои основные запасы пороха остались на корабле.');
                        } else {
                            Пистолет.Разобрать();
                        } // end if
                    }
        ...
        Порох.Действие('порох в замок', [
            ...
            {глагол: ['положить', 'засунуть', 'насыпать'], А:Порох, Б:'скважина', В:[null, {предлоги:null, слова:'замок'}]},
            //----------
            ], function () {
                if (Порох.Находится(ИГРОК) == НЕТ) {
                    if (Порох.Находится(Пистолет) == ДА) {
                        Вывести('Сперва его надо раздобыть.');
                    } else {
    ...
    // САБЛЯ
        ...
        //----------
        Сабля.Описывается(['сабля'], null, ['е']);
    ...
    // ВЕРЁВКА
        ...
        //----------
        Верёвка.Описывается(['верёвка', 'бечёвка'], null, ['е']);
        ...
        Верёвка.Действие([
            ...
            {глагол:['положить', 'засунуть'], А:Верёвка, Б:'скважина', В:[null,Замок]},
            //----------
            ], function() {
    ...
    // ФИТИЛЬ
        ...
        //----------
        Фитиль.Описывается(['фитиль'], null, ['е']);
        ...
        Фитиль.Действие('фитиль', [
            ...
            {глагол:['положить','засунуть'], А:Фитиль, Б:'скважина', В:[null,Замок]},
            //----------
            ], function() {
                if (Порох.Находится(Замок) == НЕТ) {
                    Вывести('Ну сгорит фитиль в замке - толку от этого?');
                } else {
    ...
    //--------------------------------------------------
    // ПИСЬМО
        Письмо.Описывается(['письмо', 'бумага'], null, ['е']);
        //----------
    ...
    //--------------------------------------------------
    // МЕШОЧЕК
        Мешок.Описывается(['мешок'], null, ['е']);
        //----------
    ...
    // ПИСТОЛЕТЫ
        //----------
        Пистолеты.Описывается('пистолеты', null, ['е','м']);
        //----------
        Пистолеты.Описание('пистолеты', function () {
            if (Пистолеты.Находится(Сундук) == ДА) {
                ...
                return 'Слухи о таких пистолетах до вас доходили, но видеть их вживую раньше не доводилось. Эти пистолеты заряжаются специальными пулями, сквозь которые пропущены короткие фитили. Каждый выстрел воспламеняет очередной фитиль — таким образом пистолет стреляет несколько раз.';
            } // end if
        });
        //----------
        Пистолеты.Действие([
            {глагол:[null,'осмотреть','изучить'], А:Пистолеты},
            {глагол:['осмотреть','изучить'], А:Ствол, В:Пистолеты},
            //----------
            ], function() {
        ...
        //----------
        function СтрельбаИзПистолетов (Цель) {
            // || 
            if (Пистолет.Находится(ИГРОК) == ДА) {
                Ствол.Контекст = Пистолет;
                //----------
                if (Цель != undefined && Цель.Это(ИГРОК) == ДА) {
                    Вывести('Вы пока недостаточно проголодались для таких мыслей.');
                } else {
                ...
                if (Карта.ПоказалиКутуше == НЕТ) {
                    Вывести('Отсюда ни в кого не попасть.');
                } else {
                    Вывести('Рано - они могут повернуть обратно.');
                } // end if
                //----------
                Ствол.Контекст = Пистолеты;
            } else {
                КутушПлывётОбратно.Выключить();
                Сражение.Выключить();
                Концовка.Вывести();
        ...
        //----------
        ИГРОК.Действие('пальба', [
            //{глагол:'стрелять', А:[null, {предлоги:'в', слова:'воздух'}], Б:[null,Пистолет,Пистолеты]},
            {глагол:'стрелять', А:[null, ИГРОК, 'воздух'], Б:[null,Пистолет,Пистолеты]},
            //----------
            ], function(Команда) {
                if (Команда.ФормаГлагола.substr(-2) == 'ся' || Команда.ФормаГлагола.substr(-2) == 'сь') {
                    СтрельбаИзПистолетов(ИГРОК);
                } else if (Команда.А.Объект != null && Команда.А.Объект.Это(ИГРОК)) {
                     СтрельбаИзПистолетов(ИГРОК);
                } else {
                    СтрельбаИзПистолетов();
                } // end if
    ...
    // КАРТА
        ...
        //----------
        Карта.Описывается(['карта','пергамент'], null, ['е']);
        //----------
        ...
        function СигналКораблю () {
            ...
            var текст = '';
            //----------
            if (Корабль.ЗаметилиСуматоху == НЕТ) {
                текст += '\n\n\
                Не без удовольствия вы заметили суматоху на корабле. Так, Кутуш обнаружил-таки пропажу.\n\n\
                <- далее ->\n\n';
            }
            //----------
            Вывести(текст+'Расправив карту, вы подняли её над головой так высоко, как смогли.\n\n\
                \n\n\
        ...
        Корабль.Действие('показать карту', [
            {где:ВЕЗДЕ, глагол:['махать','сигналить','привлечь'], А:[Корабль], Б:[Карта]},
        ...
        Кутуш.Действие('показать карту', [
            {где:ВЕЗДЕ, глагол:['махать','сигналить','привлечь'], А:[Кутуш,'пират'], Б:[Карта]},
        ...
        Карта.Действие('показать карту', [
            {где:ВЕЗДЕ, глагол:['махать','сигналить'], Б:[Карта]},
    ...
    // ШЛЮПКА
        ...
        Шлюпка.Действие('пальба', [
            {где:ВЕЗДЕ, глагол:'стрелять', А:[Шлюпка], Б:[null,Пистолеты]},
            //----------
            ], function() {
                СтрельбаИзПистолетов(Шлюпка);
            }
        ); // действие
        //----------
/* --------------------------------------------------------------------------- */
...
AZ.executeAfter(function() {
    ...
    Execute('осм');
    Execute('далее');
    Execute('изучить ствол');
    Execute('копать саблей');
    Execute('далее');
    Execute('привязать верёвку к сундуку');
    Execute('вытащить сундук');
    Execute('разобрать пистолет');
    Execute('засунуть порох в замок');
    Execute('отрезать кусок верёвки');
    Execute('засунуть фитиль в замок');
    Execute('зажечь фитиль');
    Execute('открыть сундук');
    Execute('осм сундук');
    Execute('взять пистолеты');
    Execute('осм письмо');
    Execute('достать карту из мешка');
    Execute('далее');
    Execute('далее');
});
